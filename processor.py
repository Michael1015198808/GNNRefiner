...
class GraphPreprocessor(object):
    def __init__(self, cons_name: str, goal_name: str, in_name: str, device, graph_name = None):
        ...
        self.process_in(in_name)
        self.process_goal(goal_name)
        self.process_cons(cons_name, cons, nodes_type)
        self.process_other(nodes_type, cons, device)

    def process_in(self, in_name):
        self.invoke_sites = []
        self.in_set = set()
    ...
    def process_goal(self, goal_name):
        self.goal_set = set()
        with open(goal_name, 'r') as f:
            for line in f.read().splitlines():
                self.goal_set.add(line)

    def process_cons(self, cons_name, cons, nodes_type):
        miss = {}
    ...
    def process_other(self, nodes_type, cons, device):
        miss = {}
        edges_type: List[int] = []
        ...
        for line in cons:
            ...
            for tail in tails:
                try:
                    ...
                    k1 = tail_type + ">" + head_type
                    k2 = tail_type + "<" + head_type
                    v = len(NODES_TYPE_DICT)
                    miss[k1] = v
                    NODES_TYPE_DICT[k1] = v
                    miss[k2] = v + 1
                    NODES_TYPE_DICT[k2] = v + 1
        if len(miss):
            print(miss)
            exit(-1)
